...
backend\.venv\Scripts\python.exe backend\manage.py dumpdata `
  ...
  --indent 2 --output nobatara-data.json
```

۳. PostgreSQL و کاربر اختصاصی بسازید و `DATABASE_URL` را در فایل env سرور قرار دهید:

```text
DATABASE_URL=postgresql://nobatara:STRONG_PASSWORD@127.0.0.1:5432/nobatara
```
...
backend\.venv\Scripts\python.exe backend\manage.py loaddata nobatara-data.json
```
